...
(\textbf{A}) Three-dimensional visualization of partition states $(n, \ell, m)$
for $n = 1$--4, colored by depth level.
(\textbf{B}) Validation of capacity formula $C(n) = 2n^2$ showing exact match
between theory (line) and enumeration (bars).
(\textbf{C}) Subshell capacities $C_\ell = 2(2\ell+1)$ for s, p, d, f, g orbitals.
(\textbf{D}) SOD1 copper electronic structure showing \ce{Cu^{2+}} 3d$^9$
configuration with unpaired electron.}
\label{fig:partition}
\end{figure*}
...
(\textbf{A}) Three-dimensional transition pathway through $(n, \ell, m)$ space
showing allowed (solid) and forbidden (dashed) transitions.
(\textbf{B}) Selection rule matrix: green indicates allowed transitions
($\Delta\ell = \pm 1$, $|\Delta m| \leq 1$), red indicates forbidden.
(\textbf{C}) Enforcement ratio showing $>10^8$ rate suppression for
forbidden transitions.
(\textbf{D}) SOD1 electron $m$-value evolution during catalytic half-cycle
...
(\textbf{A}) Three-dimensional hydrogen bond network with nodes colored by
phase and edges representing coupling.
(\textbf{B}) Order parameter $\orderpar$ evolution showing synchronization
transition from disorder to native state ($\orderpar > 0.8$).
(\textbf{C}) Coupling strength decay $K/K_0 = \exp(-r/r_0)$ with
characteristic range $r_0 = 5$~\AA.
(\textbf{D}) Polar plot of synchronized oscillator phases in native structure.}
\label{fig:phaselock}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/panel4_sentropy.png}
\caption{\textbf{S-Entropy Space and Zero-Backaction Measurement.}
(\textbf{A}) Three-dimensional trajectory through S-entropy space
$(\Sk, \St, \Se)$ showing knowledge gain (decreasing $\Sk$) balanced by
temporal uncertainty (increasing $\St$) on the constraint surface
$\Sk + \St + \Se = 1$.
(\textbf{B}) Ternary partition of S-entropy space with 9 cells labeled by
two-trit addresses.
(\textbf{C}) Conservation law: stacked plot showing $\Sk + \St + \Se = 1$
maintained throughout measurement.
(\textbf{D}) Backaction comparison: categorical measurement achieves
$\delta \sim 10^{-4}$, a 6,500-fold improvement over Heisenberg limit.}
\label{fig:sentropy}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/panel5_electron.png}
\caption{\textbf{Electron Trajectory Tracking in SOD1.}
(\textbf{A}) Three-dimensional electron trajectory through active site
during \ce{Cu^{2+}} $\to$ \ce{Cu^{+}} half-cycle, colored by time
(blue$\to$red). Orange sphere marks Cu center.
(\textbf{B}) Distance to copper showing minimum approach at 98~pm
($t = 45$~fs) during electron transfer.
(\textbf{C}) Magnetic quantum number $m$ evolution showing transitions
$+2 \to +1 \to 0 \to -1 \to 0 \to +1 \to +2$ consistent with selection rules.
(\textbf{D}) Backaction validation: measured $\delta = (1.52 \pm 0.28)
\times 10^{-4}$ per iteration.}
\label{fig:electron}
\end{figure*}
...
\caption{\textbf{Catalytic Mechanism: $\dC = 1$ Aperture Traversal.}
(\textbf{A}) Three-dimensional active site showing Cu (orange) and Zn (gray)
centers with His ligands (triangles) and superoxide approach trajectory
(dashed).
(\textbf{B}) Categorical distance comparison: SOD1, CA~II, and catalase
all achieve $\dC = 1$.
(\textbf{C}) Turnover rate scaling: $\kcat$ decreases with increasing $\dC$,
confirming categorical distance determines speed.
(\textbf{D}) Ping-pong energy diagram showing two half-reactions with
minimal barriers.}
\label{fig:catalysis}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/panel7_conformational.png}
\caption{\textbf{Conformational Dynamics of Electrostatic Loop.}
(\textbf{A}) Three-dimensional loop conformations: closed (blue,
$\orderpar = 0.92$) and open (red, $\orderpar = 0.71$) states with
Cu center (orange).
(\textbf{B}) Loop order parameter during transition showing
$\Delta\orderpar = -0.21$ over $\sim$50~ps.
(\textbf{C}) Hydrogen bond phase deviations: closed state (blue) shows
tight clustering; open state (red) shows broader distribution.
(\textbf{D}) Temperature independence: $\orderpar$ (blue) remains constant
while transition rate (orange) varies with $T$.}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/panel8_folding.png}
\caption{\textbf{Protein Folding as Trajectory Completion.}
(\textbf{A}) Three-dimensional folding trajectory in reduced coordinate
space from unfolded (red) to native (green) state.
(\textbf{B}) Order parameter $\orderpar$ during folding showing sigmoidal
transition through five stages.
(\textbf{C}) Complexity comparison: categorical approach scales as
$O(N\log_3 N)$ vs exponential $O(3^N)$ for conformational search.
Vertical line marks SOD1 ($N = 153$).
(\textbf{D}) Folding intermediates: U (unfolded) $\to$ I$_1$ $\to$ I$_2$
$\to$ I$_3$ $\to$ N (native) with increasing $\orderpar$.}
\label{fig:folding}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/panel9_disease.png}
\caption{\textbf{ALS Misfolding as Coherence Loss.}
(\textbf{A}) Three-dimensional structure comparison: wild-type (green,
ordered $\beta$-barrel) vs A4V mutant (red, disordered).
(\textbf{B}) Phase coherence by variant: WT ($\orderpar = 0.87$) vs
ALS mutants with $\orderpar < 0.5$ (below stability threshold).
(\textbf{C}) Correlation between coherence and survival time showing
exponential relationship.
(\textbf{D}) Therapeutic prediction: pharmacological chaperone restores
$\orderpar > 0.5$, potentially slowing disease progression.}
\label{fig:disease}
\end{figure*}
